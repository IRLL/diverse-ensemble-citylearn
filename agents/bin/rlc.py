...
try:
    ...
    raise Exception(
        "This functionality requires you to install torch. You can install torch by : "
        "pip install torch torchvision, or for more detailed instructions please visit https://pytorch.org.")


class RLC(Agent):
    def __init__(
            self, *args, observation_space: spaces.Box, encoders: List[Encoder] = None,
            hidden_dimension: List[float] = None,
    ...
    def hidden_dimension(self, hidden_dimension: List[float]):
        self.__hidden_dimension = [512, 512] if hidden_dimension is None else hidden_dimension

    @discount.setter
    def discount(self, discount: float):
        # self.__discount = 0.99 if discount is None else discount
        self.__discount = 0.986 if discount is None else discount
    ...
    def batch_size(self, batch_size: int):
        # self.__batch_size = 64 if batch_size is None else batch_size
    ...
    def replay_buffer_capacity(self, replay_buffer_capacity: int):
        self.__replay_buffer_capacity = int(2 ** 18) if replay_buffer_capacity is None else replay_buffer_capacity

    @start_training_time_step.setter
    def start_training_time_step(self, start_training_time_step: int):
        # self.__start_training_time_step = 8759 if start_training_time_step is None else start_training_time_step
        self.__start_training_time_step = 5*30*24 if start_training_time_step is None else start_training_time_step

    @end_exploration_time_step.setter
    def end_exploration_time_step(self, end_exploration_time_step: int):
        # self.__end_exploration_time_step = 8759 if end_exploration_time_step is None else end_exploration_time_step  # 1 episode of eval and 1 episode of random actions
        self.__end_exploration_time_step = 5*30*24 if end_exploration_time_step is None else end_exploration_time_step  # 1 episode of eval and 1 episode of random actions

    @deterministic_start_time_step.setter
    def deterministic_start_time_step(self, deterministic_start_time_step: int):
        self.__deterministic_start_time_step = np.nan if deterministic_start_time_step is None \
            else deterministic_start_time_step
    ...
    def update_per_time_step(self, update_per_time_step: int):
        update_per_time_step = 1 if update_per_time_step is None else update_per_time_step
        assert isinstance(update_per_time_step, int), \
            f'update_per_time_step mut be int type. {update_per_time_step} is of {type(update_per_time_step)} type'
